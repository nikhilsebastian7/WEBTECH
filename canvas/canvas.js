const design = async function(width,height,number)
{
    var temp_canvas=document.createElement("canvas1");
    var temp_ctx=temp_canvas.getContext("2d");
    temp_canvas.height=height;
    temp_canvas.width=width;
}
function diagonal1(ctx,x,y,s)
{
    ctx.beginPath();
    ctx.moveTo(x,y);
    ctx.lineTo(x+s,y+s);
    ctx.stroke();
    ctx.closePath();
}
//Function for forward slash(/)
function diagonal2(ctx,x,y,s)
{
    ctx.beginPath();
    ctx.moveTo(x+s,y);
    ctx.lineTo(x,y+s);
    ctx.stroke();
    ctx.closePath();
}

//Function that is used to prepare desired generatives using abc
function generative()
{
    var x=0;
    var y=0;
    var s=20;
    while(x<width)
    {
        y=0;
        while(y<height)
        {
            if(Math.random()>0.5)
                diagonal1(temp_ctx,x,y,s);
            else
                diagonal2(temp_ctx,x,y,s);
            y+=5;
        }
        x+=5;
    }
}

generative();